...
def _detect_mask(
    ...
    def on_mouse(event: int, x: int, y: int, flags: int, userdata: list[tuple[int, int]]):
    ...
    cv2.setMouseCallback("RealSense", on_mouse, clicks)
...
def track_pose(
    ...
    pose_seq = []  # 存储每一帧的 4x4
    try:
        init_mask = _detect_mask(pipeline, align)
        ...
        if init_mask is not None:
            ...
            while True:
                ...
                mask_vis_path_str = str(mask_visualization_path / f"{frame_index:04d}.png") if mask_visualization_path else None
                bbox_vis_path_str = str(bbox_visualization_path / f"{frame_index:04d}.png") if bbox_visualization_path else None
                pose_vis_path_str = str(pose_visualization_path / f"{frame_index:04d}.png") if pose_visualization_path else None
